...
Прежний `website` переименован в `website-old` и сохранён как архив. Astra-версия стала единым `website`: сохранены её главная, визуальный язык, 24 товара и локальные фото. Из прежней версии перенесены варианты со статусами наличия/предзаказа/недоступности, защищённая корзина, связанные товары и контрактная checkout-логика. На текущем CSV нет колонки `Наличие`, поэтому все 24 позиции по умолчанию показаны как «в наличии»; перед заказом авторитетен WooCommerce.
