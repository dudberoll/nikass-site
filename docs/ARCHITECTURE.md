...
Browser commerce normally has one authenticated checkout in `webapp`: `website` may provide public product
information and an anonymous local cart, but it cannot create payments or own order state. The NIKASS product
exception is one guest checkout presentation in `website`; it calls the same contract-validated backend flow,
stores no authoritative totals and creates no payments. Mobile
