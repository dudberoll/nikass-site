...
test('chat route validates the conversation and returns the provider reply', async () => {
  const app = createApp({
    env,
    prisma: {} as DbClient,
    chatProvider: {
      respond: async (messages) => `Ответ на: ${messages.at(-1)?.content}`,
    },
  })
  ...
  const response = await app.request('/api/chat', {
    ...
    body: JSON.stringify({ messages: [{ role: 'user', content: 'Какая станция подойдёт для дома?' }] }),
  })
...
test('chat route fails closed when no AI provider is configured', async () => {
  ...
  expect(await response.json()).toEqual({
    error: { code: 'INTERNAL_ERROR', message: 'Сервис консультанта временно недоступен.' },
  })
})
