...
describe('loadEnv', () => {
  ...
  test('requires both AI endpoint and key when the chat provider is enabled', () => {
    const base = {
      DATABASE_URL: 'postgresql://superuser:superpassword@localhost:54329/web_app_demo',
      JWT_SECRET: '12345678901234567890123456789012',
    ...
    expect(() => loadEnv(base)).toThrow('AI_API_URL')
    expect(() => loadEnv({ ...base, AI_API_URL: 'https://ai.example.com' })).toThrow('AI_API_KEY')
    expect(loadEnv({ ...base, AI_API_URL: 'https://ai.example.com', AI_API_KEY: 'test-key' }).AI_PROVIDER)
