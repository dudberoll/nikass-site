> Backend активен для API каталога, guest checkout и AI-консультанта; публикация пока отложена.
...
The public AI consultant is `POST /api/chat`. It accepts up to 20 user/assistant messages and forwards them to a configured chat-completions-compatible endpoint. Set `AI_PROVIDER=chat-completions`, `AI_API_URL`, `AI_API_KEY`, and `AI_MODEL`; the default `AI_PROVIDER=disabled` fails closed. Runtime questions are routed to the deterministic energy calculator tool; factual questions are sent without tools. The grounded prompt and formula live in [../docs/CHAT_KNOWLEDGE.md](../docs/CHAT_KNOWLEDGE.md). Responses are not stored, and the route has a body limit plus an in-process IP rate limit.
...
## NIKASS catalog scaffold

The public catalog is isolated in `src/modules/catalog` and does not use the
database. It exposes:

- `GET /api/catalog` with `q`, `category`, `minPrice`, `maxPrice`, `sort`,
  `page`, and `perPage` query parameters;
- `GET /api/catalog/:slug` for one product.

The provider is disabled by default. To connect WooCommerce, set
`CATALOG_PROVIDER=woocommerce`, `WOOCOMMERCE_PRODUCTS_ENDPOINT` to the REST
products endpoint (usually `/wp-json/wc/v3/products`), and fill the two
`WOOCOMMERCE_CONSUMER_*` values in the ignored `backend/.env`. The adapter keeps
credentials out of URLs, validates the upstream JSON, loads variations for
variable products, and caches the last successful catalog for up to five
minutes. See [TASKS.md](TASKS.md) for the next integration steps.
...
`POST /api/catalog/cart/review` accepts `{ "version": 1, "items": [{ "slug": "nikass-ns-31", "sku": "NS31-300-288", "quantity": 2 }] }`.
The shared schemas live in `packages/contracts/src/cart.ts`. Client prices and
unknown fields are rejected; quantities are 1–99, with 1–100 unique lines.
Every review refreshes WooCommerce data (concurrent refreshes share a request).
An unavailable provider returns 503 even if a browsing cache exists.

The response contains `checkedAt`, RUB prices in kopecks (`unitPriceMinor`,
`lineTotalMinor`, `subtotalMinor`) and each line's availability or `missing`.
Missing/unavailable lines have no line total and are excluded from the subtotal.
The subtotal excludes shipping and is not an order total. A review does not
reserve inventory, prove sufficient stock for the quantity, or create an order.
Checkout must recheck stock and prices and require acceptance of changes.
No real API access was available during implementation; adapter field mapping
must be verified against the actual store before activation.

## NIKASS guest orders

Guest address/contact validation, native WooCommerce coupons and order creation,
and durable manager email/Telegram tasks are in `src/modules/orders`.
Activation, API, migration, verification and uncertain-write recovery are in
[../docs/ORDERS.md](../docs/ORDERS.md). `ORDERS_ENABLED` defaults to false.
