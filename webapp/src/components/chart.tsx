    ...
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        className={cn(
          ...
          size === "fixed" ? "h-64 w-full" : "aspect-video",
          className
        )}
        {...props}
      >
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig } & NoStyleOverrides) => {
...
function ChartLegendContent({
  ...
  hideIcon?: boolean
  nameKey?: string
} & RechartsPrimitive.DefaultLegendContentProps & NoStyleOverrides) {
  const { config } = useChart()
