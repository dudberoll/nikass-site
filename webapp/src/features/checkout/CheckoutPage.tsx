...
export function CheckoutPage() {
  ...
  return <main className="nikass-checkout">
    ...
    {error && <Typography as="p" variant="bodySm" tone="destructive" role="alert">{error}</Typography>}
    {result?.state === 'confirmed' ? <section aria-live="polite"><Typography as="h2" variant="h2">Заказ №{result.orderNumber}</Typography><Typography as="p" variant="body">Заказ ожидает обработки менеджером. Сохраните номер для обращения.</Typography><Button variant="outline" onClick={newCart}>Перейти к новой корзине</Button></section>
      ...
      : cart && <form onSubmit={review} noValidate>
        <fieldset disabled={busy}><Typography as="legend" variant="h2">Контакты и адрес доставки</Typography><div className="checkout-fields">
          {fields.map(([name, label, type, autoComplete, maxLength]) => <div key={name}><Typography as="label" variant="label" htmlFor={name}>{label}</Typography><Input id={name} name={name} type={type} defaultValue={draft[name] ?? ''} autoComplete={autoComplete} maxLength={maxLength} required={name !== 'apartment'} aria-invalid={Boolean(fieldErrors[name])} aria-describedby={fieldErrors[name] ? `${name}-error` : undefined} />{fieldErrors[name] && <Typography as="p" variant="bodySm" tone="destructive" id={`${name}-error`}>{fieldErrors[name]}</Typography>}</div>)}
        </div><Typography as="label" variant="label" htmlFor="comment">Комментарий к заказу</Typography><Textarea id="comment" name="comment" defaultValue={draft.comment ?? ''} required maxLength={1000} aria-invalid={Boolean(fieldErrors.comment)} aria-describedby="comment-error" placeholder="Например, когда удобнее позвонить" /><Typography as="p" variant="bodySm" tone="destructive" id="comment-error">{fieldErrors.comment}</Typography>
        ...
        <label className="checkout-consent"><input type="checkbox" name="consent" defaultChecked={draft.consent === 'on'} required aria-invalid={Boolean(fieldErrors.consent)} /><Typography as="span" variant="body">Согласен с {termsUrl ? <Typography as="a" variant="link" href={termsUrl} target="_blank" rel="noreferrer">условиями покупки</Typography> : 'условиями покупки'} и {privacyUrl ? <Typography as="a" variant="link" href={privacyUrl} target="_blank" rel="noreferrer">обработкой персональных данных</Typography> : 'обработкой персональных данных'}.</Typography></label><Typography as="p" variant="bodySm" tone="destructive">{fieldErrors.consent}</Typography>
