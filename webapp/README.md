> Настройка этой поверхности отложена до уточнения личного кабинета и оплат. Сейчас выбран публичный сайт и только локальный запуск (2026-09-07).
...
`/checkout` is the sole guest order form (explicit NIKASS exception to the
account-first template). It imports the website cart fragment, validates contacts
and address, displays the server coupon/price quote and preserves the attempt
across reloads. Configure the API and approved legal URLs; see
[../docs/ORDERS.md](../docs/ORDERS.md). `bun run e2e:checkout` tests browser recovery
without Docker using mocked API responses.
